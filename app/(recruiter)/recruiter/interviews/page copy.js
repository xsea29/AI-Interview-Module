...
const Interviews = () => {
  ...
  const fetchInterviews = async () => {
    try {
      ...
      const params = new URLSearchParams({
        ...
        limit: '50',
        sort: '-createdAt',
        populate: 'candidateId,jobId,recruiterId'
      });

      const response = await fetch(
        `${API_BASE_URL}/interviews?${params}`,
        {
          headers: {
  ...
  const handleCancelInterview = async (interviewId, candidateName) => {
  ...
  const filteredInterviews = interviews.filter(interview => {
    ...
    if (statusFilter !== "all") {
      if (statusFilter === "needs_decision") {
        if (interview.status !== INTERVIEW_STATUS.COMPLETED || interview.decision?.status) {
          return false;
        }
      } else if (statusFilter === "pending") {
        if (![INTERVIEW_STATUS.SETUP, INTERVIEW_STATUS.QUESTIONS_GENERATED].includes(interview.status)) {
          return false;
        }
      } else if (statusFilter === "scheduled") {
        if (![INTERVIEW_STATUS.READY, INTERVIEW_STATUS.SCHEDULED].includes(interview.status)) {
          return false;
        }
  ...
  const scheduledInterviews = filteredInterviews.filter(i => 
    [INTERVIEW_STATUS.SCHEDULED, INTERVIEW_STATUS.READY].includes(i.status)
  );
  ...
  const cancelledInterviews = filteredInterviews.filter(i => 
    [INTERVIEW_STATUS.CANCELLED, INTERVIEW_STATUS.EXPIRED, INTERVIEW_STATUS.ABANDONED, INTERVIEW_STATUS.FAILED].includes(i.status)
  );
  const setupInterviews = filteredInterviews.filter(i => 
    [INTERVIEW_STATUS.SETUP, INTERVIEW_STATUS.QUESTIONS_GENERATED].includes(i.status)
  );
  ...
  const renderInterviewCard = (interview, index) => {
    ...
    const isSelectable = [INTERVIEW_STATUS.SCHEDULED, INTERVIEW_STATUS.READY].includes(interview.status);
        ...
        <Card className="hover:shadow-md transition-shadow border border-border/50">
          <CardContent className="p-4">
            <div className="flex items-center gap-4">
              ...
              {/* Score (for completed) */}
              {score !== undefined && interview.status === INTERVIEW_STATUS.COMPLETED && (
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56">
                  ...
                  {/* Setup Actions */}
                  {[INTERVIEW_STATUS.SETUP].includes(interview.status) && (
                    <>
                  ...
                  {[INTERVIEW_STATUS.QUESTIONS_GENERATED].includes(interview.status) && (
                    <>
